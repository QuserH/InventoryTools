...
namespace InventoryTools.Lists
{
    public class ListService : DisposableMediatorSubscriberBase, IListService
    {
        ...
        private void ValidateAndInjectListColumns(FilterConfiguration list)
        {
            ...
            // The retrieval action belongs beside the inventory/retainer item table. Migrate
            // configurations created by the brief craft-column implementation back to Columns.
            if (list.FilterType == FilterType.CraftFilter && list.CraftColumns != null)
            {
                var legacyColumns = list.CraftColumns
                    .Where(c => c.ColumnName == nameof(CraftAmountAvailableColumn))
                    .ToList();
                foreach (var legacyColumn in legacyColumns)
                {
                    list.CraftColumns.Remove(legacyColumn);
                    list.Columns ??= new List<ColumnConfiguration>();
                    if (!list.Columns.Any(c => c.ColumnName == legacyColumn.ColumnName))
                        list.Columns.Add(legacyColumn);
                }
            }

            // Add the required/missing quantity columns to existing craft configurations so the
            // inventory panel shows them without manual reconfiguration.
            if (list.FilterType == FilterType.CraftFilter && list.Columns != null)
            {
                var newColumnTypes = new[] { typeof(CraftRequiredQuantityColumn), typeof(CraftMissingQuantityColumn) };
                var insertAfter = list.Columns.FindIndex(c => c.ColumnName == nameof(CraftAmountAvailableColumn));
                foreach (var columnType in newColumnTypes)
                {
                    if (list.Columns.Any(c => c.ColumnName == columnType.Name))
                        continue;
                    ...
                    if (insertAfter >= 0 && insertAfter < list.Columns.Count)
                    {
                        insertAfter++;
                        list.Columns.Insert(insertAfter, newColumn);
                    }
                    else
                    {
                        list.Columns.Add(newColumn);
                    }
                }
            }
        }
        ...
        public void AddDefaultColumns(FilterConfiguration configuration)
        {
            ...
            else if (configuration.FilterType == FilterType.CraftFilter)
            {
                ...
                AddColumn(configuration,typeof(CraftRequiredQuantityColumn), false);
                AddColumn(configuration,typeof(CraftMissingQuantityColumn), false);
