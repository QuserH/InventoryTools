...
public class CraftTrackerTrackMarketBoardFilter : BooleanFilter
{
    ...
    public override string Name { get; set; } = LocalizationService.Ui(LocalizationService.Ui("Track Market Board?"));

    public override string HelpText { get; set; } =
        LocalizationService.Ui(LocalizationService.Ui("When you buy something off the market board and it matches one of the output items in this craft list, should it reduce the quantity of that item? The craft list must be active for this to count."));
