...
namespace InventoryTools.Ui.Pages
{
    public class CharacterRetainerPage : Page
    {
        ...
        public override string Name { get; } = LocalizationService.Ui("Characters/Retainers");
        ...
        public override List<MessageBase>? Draw()
        {
            ...
            using (var sidebar = ImRaii.Child("charactersBar", new Vector2(160, 0) * ImGui.GetIO().FontGlobalScale, true))
            {
                if (sidebar.Success)
                {
                    ...
                    ImGui.TextUnformatted(LocalizationService.Ui("Characters (") + characters.Count + ")");
                    ImGui.Separator();
                    ...
                    ImGui.TextUnformatted(LocalizationService.Ui("Free Companies (") + freeCompanies.Count + ")");
                    ImGui.Separator();
                    ...
                    ImGui.TextUnformatted(LocalizationService.Ui("Residences (") + houses.Count + ")");
                    ImGui.Separator();
                    ...
                    ImGui.TextUnformatted(LocalizationService.Ui("Retainers (") + retainers.Count + ")");
                    ImGui.Separator();
                    ...
                    {
                        ImGui.TextUnformatted(LocalizationService.Ui("Orphaned Retainers:"));
                        ImGui.Separator();
                    ...
                    ImGui.Text(LocalizationService.Ui("World: "));
                    using var combo = ImRaii.Combo(LocalizationService.Ui("##activeWorld"), selectedWorld?.Name.ExtractText() ?? LocalizationService.Ui("All"));
                    if (combo.Success)
                    {
                ...
                if (main.Success)
                {
                    if (_selectedCharacter != 0)
                    {
                        ...
                        if (character != null)
                        {
                            ...
                            ImGuiUtil.HoverTooltip(LocalizationService.Ui("Edit name, set the name to blank to return it to the original name."));

                            if (_editMode)
                            {
                                var newName = _newName;
                                ImGui.Text(LocalizationService.Ui("Custom Name: "));
                                ImGui.SameLine();
                                if (ImGui.InputText(LocalizationService.Ui("##customName"), ref newName, 100))
                                {
                                    _newName = newName;
                                }
                                ...
                                {
                                    ImGui.Text(LocalizationService.Ui("Original Name: ") + character.Name);
                                }
                            ...
                            {
                                ImGui.Text(LocalizationService.Ui("Level: ") + character.Level);
                                ImGui.Text(LocalizationService.Ui("Gil: ") + character.Gil);
                                ImGui.Text(LocalizationService.Ui("Gender: ") + character.Gender);
                                ImGui.Text(LocalizationService.Ui("Free Company: ") + character.FreeCompanyName);
                                ImGui.Text(LocalizationService.Ui("World: ") + (character.World?.Name.ExtractText() ?? "Unknown"));
                                ImGui.Text(LocalizationService.Ui("Class/Job: ") +
                            ...
                            {
                                ImGui.Text(LocalizationService.Ui("World: ") + (character.World?.Name.ExtractText() ?? "Unknown"));
                                ImGui.Text(LocalizationService.Ui("Plot Size: ") + character.GetPlotSize());
                                ImGui.Text(LocalizationService.Ui("Location: ") + character.HousingName);
                                ImGui.Text(LocalizationService.Ui("Owners: "));
                            ...
                            {
                                ImGui.Text(LocalizationService.Ui("World: ") + (character.World?.Name.ExtractText() ?? "Unknown"));
                                ImGui.Text(LocalizationService.Ui("Related Characters: "));
                            ...
                            ImGui.NewLine();
                            ImGui.Text(LocalizationService.Ui("Inventories: "));
                            ImGui.Separator();
                            var inventories =
                            ...
                            {
                                ImGui.Text(LocalizationService.Ui("No inventories found."));
                            }

                        }
                        else
                        {
                            ImGui.Text(LocalizationService.Ui("Invalid character selected."));
                        }
                    }
                }
            }
